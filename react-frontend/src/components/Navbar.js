    ...
    >
      ...
      {!loginInfo ? (
        ...
        <div className="dropdown">
          ...
          <div className={`${classes.logout} d-flex gap-3 align-items-center`}>
            <p className="mb-0">{`@${loginInfo.username}`}</p>
            <button className={`btn fs-5 p-1 ${classes.logbtn}`} type="submit" title="logout"><i class="bi bi-box-arrow-right"></i></button>
          </div>
          </Form>
        </div>
      )}
